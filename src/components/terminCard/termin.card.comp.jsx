    ...
    <section className="flex justify-center flex-col mt-16 items-center min-h-screen px-4 sm:px-8">
      <div className="card bg-base-100 w-full sm:w-96 shadow-sm">
        <div className="card-body justify-center items-center">
          ...
          <p>
            Plane hier deine Termine, damit dir nichts mehr durchrutscht. 😊
          </p>
            ...
            {Object.entries(months).map(([month, weeks]) => {
              ...
              return (
                <div key={month} className="mb-8">
                  ...
                  >
                    <span>
                      {new Date(0, month).toLocaleString("de-DE", {
                        month: "long",
                      })}
                    </span>
                    {/* TODO style änder */}
                  ...
                  {openMonth === month && (
                    <table className="table-auto w-full border-collapse sm:block">
                      <thead className="hidden sm:table-header-group">
                        <tr>
                          <th className="border p-2 sm:p-4">KW</th>
                          <th className="border p-2 sm:p-4">Mo</th>
                          <th className="border p-2 sm:p-4">Di</th>
                          <th className="border p-2 sm:p-4">Mi</th>
                          <th className="border p-2 sm:p-4">Do</th>
                          <th className="border p-2 sm:p-4">Fr</th>
                          <th className="border p-2 sm:p-4">Sa</th>
                          <th className="border p-2 sm:p-4">So</th>
                        </tr>
                      </thead>
                      <tbody>
                        {Object.entries(weeks).map(([kw, weekdays]) => (
                          <tr
                            key={kw}
                            className={`text-center align-top ${
                              parseInt(kw) === currentKW &&
                              parseInt(year) ===
                                // TODO color anpassen
                                currentYear
                          ...
                          >
                            <td className="border p-2 sm:p-4 ">KW {kw}</td>
                            {weekdays.map((dayMemos, dayIdx) => (
                              <td
                                key={dayIdx}
        ...
        {sortedMemos.map((memo, index) => (
          <div key={index} className="mb-4">
            <SavedCard
              memo={memo}
              onDelete={() => {
                const updated = savedMemos.filter((_, i) => i !== index);
                setSavedMemos(updated);
              }}
            />
          </div>
